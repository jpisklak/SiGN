test_that("SiGN_eval returns correct structure", {
...
test_that("errors are thrown for mismatched lengths", {
  expect_error(SiGN_eval(1:5, 1:4), "different lengths")
})
...
test_that("warning/message is triggered for small sample size", {
  ...
  expect_message(SiGN_eval(obs, pred), "Sample size is small")
})
...
test_that("epsilon affects log-likelihood", {
  ...
  ll1 <- SiGN_eval(obs, pred, epsilon = 0.001)$info_criteria$logLik
  ll2 <- SiGN_eval(obs, pred, epsilon = 0.0001)$info_criteria$logLik
