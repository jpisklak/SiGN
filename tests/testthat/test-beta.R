test_that("beta returns 1 when use_beta or sig is FALSE", {
  expect_equal(
    beta(FALSE, TRUE, "VI", "VI", 10, 20, 5),
    1
  )

  expect_equal(
    beta(TRUE, FALSE, "VI", "VI", 10, 20, 5),
    1
  )

  expect_equal(
    beta(FALSE, FALSE, "VI", "VI", 10, 20, 5),
    1
  )
})

test_that("beta computes correct value for VI-VI schedules", {
  result <- beta(TRUE, TRUE, "VI", "VI", 10, 20, 5)
...
test_that("beta uses il_dur1 when schedules are not both VI", {
  ...
  result <- beta(TRUE, TRUE, "FR", "VI", 10, 20, 5)
