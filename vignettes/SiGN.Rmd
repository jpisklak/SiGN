...
Several decades ago, researchers first reported that pigeons (*Columba livia*) sometimes prefer options that yield less food over those that provide more [@Kendall1974]. This behaviour—often described as suboptimal or paradoxical—reduces overall food intake in controlled settings. 
...
For a detailed description of the model and its calculations, see @Dunn2024.
...
> (Notably, it is preferred by the author of this R package—but not his colleagues 😁)
...
To illustrate, consider the experimental setup from Stagner & Zentall -@StagnerZentall2010, shown in Figure 1.
...
This procedure involves 16 parameters to describe the contingency, plus 2 additional parameters specific to the SiGN model—making a total of 18. To speed up the process of entering these values, the `choice_params()` function includes several pre-defined profiles: `"zentall"` [@StagnerZentall2010], `"kendall"` [@Kendall1985], and `"fantino"` [@Fantino1969].
...
knitr::kable(
  data.frame(
    ...
    Description = c(
      ...
      "Logical toggle to adjust conditional vs. primary reinforcement (Equation 6, Dunn et al., 2024)."
    )
  ),
...
An in-depth discussion of the output in `$details` is beyond the scope of this document and readers interested are directed to @Dunn2024. However, a brief description of each is provided in Table 2.

An in-depth explanation of the `$details` output is beyond the scope of this document. Readers interested in the theoretical rationale behind these terms are referred to @Dunn2024. However, a brief description of each component is provided in Table 2 below.
...
knitr::kable(
  ...
  caption = "Table 2: Components of the `SiGN()` Model Output (`$details`)",
  escape = FALSE
)
```
