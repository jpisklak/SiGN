...
The SiGN model's predicted choice proportion, for each row of `subopt_avian[9:24]`, can be called with `preds$cp`. These can now be compared directly to the observed choice proportions stored in `subopt_avian$cp`. For more information about the `SiGN()` function and its outputs, see the [Get Started](articles/SiGN.html) article.

## Visualising the Predictions

To help visualise the model’s performance, we’ll fit a simple linear regression to the observed versus predicted choice proportions. This provides a clear visual summary of their relationship and allows us to overlay both a fitted regression line and a 1:1 reference line. The closer the data points fall to the 1:1 line, the more closely the model’s predictions match the observed values—offering a quick and intuitive check on model accuracy.
