---
title: "Revisiting Fantino (1969) and Squires & Fantino (1971) with the SiGN Model"
...
## Reproducing Predictions from Fantino -@Fantino_1969
...
For example, consider the contingency arrangement from Fantino's (1969) seminal paper, which was designed to illustrate how delay reduction can conditionally reinforce choice behaviour [@Fantino_1969].

<br>

```{r, echo=FALSE, out.width='30%', fig.align='center', fig.cap = 'Figure 1. Fantino (1969) Contingencies'}
knitr::include_graphics("images/fantino_1969.png")
```
...
... we can see that we obtain a choice proportion of 0.9, which is what Fantino predicted with his original delay reduction hypothesis [@Fantino_1969].

<br>

## Reproducing Predictions from Squires and Fantino -@Squires_Fantino_1971

A key feature of the SiGN model is that it does not rely solely on delay reduction to predict choice behaviour. Instead, it incorporates the approach proposed by Squires and Fantino [@Squires_Fantino_1971], in which delay reduction is multiplied by the rate of terminal reinforcement. In classic concurrent-chain arrangements—with a single, deterministic terminal link and outcome per alternative—this formulation yields predictions that align closely with theirs.
...
In the first example, Squires and Fantino compared chains with identical terminal links (TLs) but different fixed IL durations: Chain VI$x$ VI30 versus Chain VI$x$ VI90.
...
In the second example, the comparison is between Chain VI60 VI15 and Chain VI$x$ VI15. Here, the terminal links are held constant (15 s on both sides), while only the initial link duration of one alternative is manipulated. Critically, a prediction based purely on delay reduction would predict indifference (0.5) in this situation - hence the importance of @Squires_Fantino_1971 model.

In the second example, the comparison is between Chain VI60 VI15 and Chain VI$x$ VI15. In this case, the terminal links are identical—each lasting 15 seconds—while only the initial link duration of one alternative is varied. Importantly, a model based solely on delay reduction would predict indifference (i.e., a choice proportion of 0.5) because the two terminal links represent the same degree of improvement to terminal reinforcement. This highlights the significance of Squires and Fantino’s [@Squires_Fantino_1971] extension to the delay-reduction model, which incorporates both delay-reduction and reinforcement rate to explain preference.
