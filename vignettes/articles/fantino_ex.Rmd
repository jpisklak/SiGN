...
For example, consider the contingency arrangement from Fantino's (1969) seminal paper, which was designed to illustrate how delay reduction can conditionally reinforce choice behaviour (Fantino, 1969).
...
A key feature of the SiGN model is that it does not rely solely on delay reduction to predict choice behaviour. Instead, it incorporates the approach proposed by Squires and Fantino (1971), in which delay reduction is multiplied by the rate of terminal reinforcement. In classic concurrent-chain arrangements—with a single, deterministic terminal link and outcome per alternative—this formulation yields predictions that align closely with theirs.
